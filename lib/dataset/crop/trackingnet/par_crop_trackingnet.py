...
eps = 1e-5
def crop_video(dataDir, video_f, video, crop_path, instanc_size):
    video_crop_base_path = join(crop_path, video_f, 'frames', video)
    if not isdir(video_crop_base_path): makedirs(video_crop_base_path)

    sub_set_base_path = join(dataDir, video_f)
    video_base_path = join(sub_set_base_path, 'frames', video)

    #gts_path = join(video_base_path, 'groundtruth.txt')    
    gts_path = join(sub_set_base_path, 'anno', f'{video}.txt')
...
def main(dataDir='.', dataCropDir='./trackingnet_cropped', instanc_size=511, num_threads=24):
    crop_path = join(dataCropDir, 'crop{:d}'.format(instanc_size))
    if not isdir(crop_path): makedirs(crop_path)

    videos_fathers = ['TRAIN_0', 'TRAIN_1', 'TRAIN_2', 'TRAIN_3']

    for video_f in videos_fathers:
        
        videos_sons = sorted(listdir(join(dataDir, video_f, 'frames')))
        n_videos = len(videos_sons)

        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [executor.submit(crop_video, dataDir, video_f, video, crop_path, instanc_size) for video in videos_sons]
